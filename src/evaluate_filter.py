...
def evaluate_filter(ground_truth, kalman, kalman_filter): 
    ...
    if ground_truth_data.empty is False and kalman_data.empty is False:
        ...
        rtheta = ground_truth_data['theta'] - kalman_data['theta']
        
        rphi = ground_truth_data['phi'] - kalman_data['phi']
        ...
        for i in range(len(ground_truth_data['timestep'])): 
            meaned_filter_std.append((np.sqrt(var_theta[i]) + np.sqrt(var_phi[i]))/2)
            meaned_real_std.append((rtheta[i]+rphi[i])/2)
        ...
        return mse, mse_db, error_in_place, meaned_variance, kalman_filter.crashed, kalman_filter.crashed_at